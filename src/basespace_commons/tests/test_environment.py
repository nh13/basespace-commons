...
class TestEnvironment(unittest.TestCase):
    ...
    @staticmethod
    def build_default():
        root_dir = Path(tempfile.mkdtemp(prefix='tmp.', suffix='.root_dir')).resolve()
        env = Environment(
    ...
    def test_input_sample_dir(self):
        ...
        sample_dir = env.root_dir() / 'input' / 'samples' / sample_ids[0] / 'Data' / 'Intensities' / 'BaseCalls'
        sample_dir.mkdir(parents=True)
        self.assertEqual(env.input_sample_dir(sample_idx=0, try_alternate=False), sample_dir.resolve())
        with self.assertRaises(Exception):
            env.input_sample_dir(sample_idx=1, try_alternate=False)

        # make sample directory for sample 1, with alternate
        sample_dir = env.root_dir() / 'input'/  'samples' / sample_ids[1]
        sample_dir.mkdir(parents=True)
        with self.assertRaises(Exception):
            env.input_sample_dir(sample_idx=1, try_alternate=False)
        self.assertEqual(env.input_sample_dir(sample_idx=1, try_alternate=True), sample_dir.resolve())
    ...
    def test_input_sample_fastqs(self):
        ...
        for sample_idx, sample_name_and_id in enumerate(env.sample_names_and_ids()):
            ...
            sample_dir = env.root_dir() / 'input' / 'samples' / sample_id / 'Data' / 'Intensities' / 'BaseCalls'
            sample_dir.mkdir(parents=True)

            f1 = (sample_dir / (sample_name + Environment.fq_ext(end=1))).resolve()
            f2 = (sample_dir / (sample_name + Environment.fq_ext(end=2))).resolve()
            ...
            f2buggy = sample_dir / (sample_name + ".buggy" + Environment.fq_ext(end=1))
